...
#python test14.3.py -p mobilenet_ssd/MobileNetSSD_deploy.prototxt -m mobilenet_ssd/MobileNetSSD_deploy.caffemodel -v Crowd-Activity-All.avi
...
while True:
		...
		for i in np.arange(0, detections.shape[2]):
			...
			if confidence > args["confidence"]:
				...
				if not within_bounds(points, [((startX + endX)/2, (startY + endY)/2)]):
					print("not", startX, startY, endX, endY)
					continue
	...
	# multiple objects
		...
		for (k, l) in zip(range(len(trackers)), labels):
			t = trackers[k]
			...
			cv2.rectangle(frame, (startX, startY), (endX, endY),
				(0, 255, 0), 2)
			cv2.putText(frame, str(conf[k]), (startX, startY - 15),
				cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 2)
